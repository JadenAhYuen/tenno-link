...
const state = {profile:{raw:{},normalized,recommended:normalized,compact:normalized,lastSyncAt:previewNow,nextAllowedSyncAt:previewNow+5*60*1000},world:{raw:{earthCycle:{isDay:true,expiry:new Date(previewNow+45*60*1000).toISOString()},cetusCycle:{isDay:false,expiry:new Date(previewNow+18*60*1000).toISOString()},vallisCycle:{isWarm:false,expiry:new Date(previewNow+9*60*1000).toISOString()},cambionCycle:{active:"Vome",expiry:new Date(previewNow+27*60*1000).toISOString()},fissures:[{tier:'Lith',node:'Taveuni (Kuva Fortress)',missionType:'Exterminate',expiry:new Date(previewNow+16*60*1000).toISOString()}],alerts:[{node:'Mercury',expiry:new Date(previewNow+8*60*1000).toISOString()}],sortie:{boss:'General Sargas Ruk',expiry:new Date(previewNow+6*60*60*1000).toISOString()},nightwave:{activeChallenges:Array.from({length:10},(_,i)=>({id:i})),expiry:new Date(previewNow+165*86400000).toISOString()},arbitration:{node:'SolNode000',expiry:new Date(previewNow+100*365*86400000).toISOString()},archonHunt:{boss:'Archon Boreal',expiry:new Date(previewNow+85*60*60*1000).toISOString()},steelPath:{currentReward:{name:'50,000 Kuva'}},voidTrader:{active:false,activation:new Date(previewNow+194*60*60*1000).toISOString()}},lastSyncAt:previewNow,nextAllowedSyncAt:previewNow+60*1000},items:{index,schemaVersion:5,lastSyncAt:previewNow,craftables:vm.runInContext('TennoInventory.compactCatalog(catalogPayload)',context)}};
