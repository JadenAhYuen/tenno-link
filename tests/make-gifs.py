...
gif('export.gif', export, [1200, 1000, 1300, 1300, 2200])
...
def make_banner(label, subtitle, screen):
    banner = Image.new('RGB', (1100, 360), '#08151d')
    draw = ImageDraw.Draw(banner)
    for y in range(360):
        draw.line((0, y, 1100, y), fill=(8 + y // 55, 21 + y // 22, 29 + y // 30))
    preview = screen.resize((420, round(screen.height * 420 / screen.width)), Image.Resampling.LANCZOS)
    banner.paste(preview, (680, -45))
    draw.line((659, 20, 659, 340), fill='#487e88', width=1)
...
banners = [make_banner(label, subtitle, screen) for label, subtitle, screen in [
    ...
    ('MAKE A BETTER PLAN', 'Name a farm and check current sources', export[3]),
]]
gif('tenno-link-banner.gif', banners, [1800, 1800, 1800], width=1100)
mission_banners = [make_banner(label, subtitle, screen) for label, subtitle, screen in [
