...
Captured from the running Tenno Link interface on September 24, 2026 at a 680 × 860 CSS pixel viewport. The GIFs use browser screenshots of the local preview. The banner combines those captures with text; it does not show a live account.
...
3. Review the GIFs here before committing them. The script uses `tests/preview.cjs` and captures Overview scrolling, Star Chart search and filter actions, Live countdowns, and Export mode selection.
