...
Open Tenno Link to check for fresh profile and event data. The extension reads the local Warframe profile identifier and requests profile-view data directly when a refresh is due. Press **↻** to request a fresh fetch at any time.
...
Fissure and event countdowns tick locally every second while Tenno Link is visible. The interface refreshes public event data after its one-minute cache period; profile refreshes are spaced at least five minutes apart and also respect the profile endpoint's cache interval. Closing the floating interface stops its refresh checks.
