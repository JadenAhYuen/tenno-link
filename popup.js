...
function nextMissionPanel() {
  ...
  return `<div class="panel"><div class="section-title"><div><h2>Missions to explore next</h2><small>Unplayed nodes on planets with a recorded cleared junction</small></div></div>${candidates.length ? `<div class="mission-suggestions">${candidates.map(({planet,node})=>`<div class="mission-suggestion">${missionThumb(node)}<span><b>${escapeHtml(node.name)}</b><small>${escapeHtml(planet.name)} · ${escapeHtml(missionInfo(node).title)} · enemy ${node.minEnemyLevel ?? '—'}–${node.maxEnemyLevel ?? '—'}</small></span></div>`).join('')}</div><p class="muted">Your profile may omit unlocked routes; confirm access in game.</p>` : `<p class="muted">Sync your profile to find approachable nodes near recorded junction progress.</p>`}<button class="secondary-action" data-open-chart>Open Star Chart</button></div>`;
}
